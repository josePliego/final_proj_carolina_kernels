...
## Introduce Challenge/Objective of the App
The NFL, through its Next Gen Stats (NGS) initiative, collects mountains of player data every Sunday. 
Each year, the organization releases portions of this archive as part of an annual analytics
competition coined the "Big Data Bowl".

This year's prompt asks participants to investigate special teams play. For those unfamiliar with North
American football, four distinct play types compromise special teams: kickoffs, punts, extra points, and
field goals. Each team has a different set of players that are involved in these "special" play types.

Included with the prompt was data from all special teams plays from 2018-2020, including player movement/location
data, PFF scouting data, and metadata regarding each play (time of game, outcome, team, etc.).
...
Part of the final shiny application includes animated ggplot graphs showcasing the player location data
 the NFL tracks every Sunday. For each distinct special teams play types (extra point, kickoff, punt, and field goal)
...
## Punt
```{r, punt}
punt_outcomes = plays %>% filter(specialTeamsPlayType == "Punt" & gameId > 2020000000) %>% group_by(specialTeamsResult) %>% slice(n = 1)
# add extra columns for gif processing
...
## Kickoff
```{r}
kickoff_outcomes = plays %>% filter(specialTeamsPlayType == "Kickoff" & gameId > 2020000000) %>% group_by(specialTeamsResult) %>% slice(n = 1)
# add extra columns for gif processing
...
## Field Goal
```{r}
field_goal_outcomes = plays %>% filter(specialTeamsPlayType == "Field Goal" & gameId > 2020000000) %>% group_by(specialTeamsResult) %>% slice(n = 1)
# add extra columns for gif processing
