...
#'   needed for a first down} \item{Distance needed for a first down}{Team
...
#'   \item{penaltyJerseyNumber}{Jersey number and team code of the player
...
#'   college} \item{Position}{Player position} \item{Player position}{Player
#'   name} }
#' @source \url{https://www.kaggle.com/c/nfl-big-data-bowl-2022/data}
"PLAYERS"
...
#'   Accurate Snap, text)} \item{operationTime}{Timing from snap to kick on punt
#'   plays in seconds: } \item{hangTime}{Hangtime of player's punt or kickoff
#'   attempt in seconds. Timing is taken from impact with foot to impact with
#'   the ground or a player. (numeric)} \item{kickType}{Kickoff or Punt Type.
#'   Possible values for kickoff plays:,D: Deep - your normal deep kick with
#'   decent hang time,F: Flat - different than a Squib in that it will have some
#'   hang time and no roll but has a lower trajectory and hang time than a Deep
#'   kick off,K: Free Kick - Kick after a safety,O: Obvious Onside - score and
#'   situation dictates the need to regain possession. Also the hands team is on
#'   for the returning team,P: Pooch kick - high for hangtime but not a lot of
#'   distance - usually targeting an upman,Q: Squib - low-line drive kick that
#'   bounces or rolls considerably, with virtually no hang time,S: Surprise
#'   Onside - accounting for score and situation an onsides kick that the
#'   returning team doesn’t expect. Hands teams probably aren't on the field,B:
#'   Deep Direct OOB - Kickoff that is aimed deep (regular kickoff) that goes
#'   OOB directly (doesn't bounce),Possible values for punt plays:,N: Normal -
#'   standard punt style,R: Rugby style punt,A: Nose down or Aussie-style punts}
...
#'   perspective (L: Left, R: Right, C: Center, text).}
#'   \item{missedTacklers}{Jersey number and team code of player(s) charged with
...
#'   play are separated by a } \item{assistTacklers}{Jersey number and team code
#'   of player(s) assisting on the tackle. Multiple assist tacklers on a play
#'   are separated by a} \item{tacklers}{Jersey number and team code of player
...
#'   \item{puntRushers}{ersey number and team code of player(s) on the punt
